...
function getCurrentUserId(req) {
  ...
  if (serverUserId) {
    ...
    if (clientUserId && clientUserId !== serverUserId) {
      console.warn(
        `[用户身份] ⚠️ 警告：前端传入的 userId (${clientUserId}) 与服务端不一致 (${serverUserId})，使用服务端值`
      );
    }
    return serverUserId;
  }
  ...
  if (clientUserId) {
    console.warn(
      "[用户身份] ⚠️ 服务端无 session/cookie，使用前端传入的 userId（不安全）:",
      clientUserId
    );
    return clientUserId;
  }
...
function normalizeUserId(userId) {
  ...
  console.warn(
    `[userId转换] 无数字部分，使用哈希: "${userIdStr}" -> ${hashedId}`
  );
  return hashedId;
}
...
router.get("/api/getUserCoins", function (req, res) {
  ...
  conn.query(sqlStr, [userId], (error, result, field) => {
    if (error) {
      logCoinError("获取金币信息", error, {
        userId,
        originalUserId: req.query.userId,
      });
      res.json({
        error_code: 1,
        message: "获取金币信息失败",
...
router.get("/api/checkSigninStatus", function (req, res) {
  ...
  // 查询用户金币信息
  let sqlStr =
    "SELECT coin_id, user_id, last_checkin_date, continuous_days FROM t_user_coins WHERE user_id = ? LIMIT 1;";
  conn.query(sqlStr, [userId], (error, result, field) => {
    if (error) {
...
router.post("/api/userCheckin", function (req, res) {
  ...
  conn.query(sqlStr, [userId], (error, result, field) => {
    ...
    // 更新用户金币信息
    sqlStr =
      "UPDATE t_user_coins SET coin_balance = ?, total_earned = ?, last_checkin_date = ?, continuous_days = ? WHERE user_id = ?;";
    conn.query(
      sqlStr,
      [newBalance, newTotalEarned, today, continuousDays, userId],
      (error, result, field) => {
        if (error) {
          logCoinError("更新金币信息", error, {
            userId,
            newBalance,
            continuousDays,
          });
        ...
        // 记录金币变动
        sqlStr =
          "INSERT INTO t_coin_records(user_id,coin_change,change_type,change_reason,balance_after) VALUES(?,?,?,?,?);";
        conn.query(
          sqlStr,
          [
            userId,
...
router.get("/api/getUserCoinRecords", function (req, res) {
  ...
  let sqlStr =
    "SELECT * FROM t_coin_records WHERE user_id = ? ORDER BY created_at DESC LIMIT ?;";
...
router.get("/api/checkTodayCheckin", function (req, res) {
  ...
  let today = new Date().toISOString().split("T")[0];
  let sqlStr =
    "SELECT last_checkin_date FROM t_user_coins WHERE user_id = ? LIMIT 1;";
  conn.query(sqlStr, [userId], (error, result, field) => {
    if (error) {
...
router.get("/api/admin/getCurrentPageUserCoins", function (req, res) {
  ...
  let sqlStr =
    "SELECT u.user_id, u.user_name, u.phone, u.avatar, c.coin_balance, c.total_earned, c.continuous_days, c.last_checkin_date FROM t_user u LEFT JOIN t_user_coins c ON u.user_id = c.user_id WHERE u.user_name LIKE ? ORDER BY u.user_id;";
  let total;
  conn.query(sqlStr, ["%" + input + "%"], (error, result, field) => {
    if (error) {
      console.log(error);
  ...
  sqlStr =
    "SELECT u.user_id, u.user_name, u.phone, u.avatar, c.coin_balance, c.total_earned, c.continuous_days, c.last_checkin_date FROM t_user u LEFT JOIN t_user_coins c ON u.user_id = c.user_id WHERE u.user_name LIKE ? ORDER BY u.user_id LIMIT ?,?;";
  conn.query(
    sqlStr,
    ["%" + input + "%", start, pageSize],
    (error, result, field) => {
...
router.post("/api/admin/updateUserCoins", function (req, res) {
  let { userId, coinChange, changeReason } = req.body;
  ...
  conn.query(sqlStr, [userId], (error, result, field) => {
    ...
    if (!result[0]) {
      ...
      conn.query(
        sqlStr,
        [userId, initialBalance, initialEarned, 0],
        (error, result, field) => {
          if (error) {
            console.log(error);
          ...
          conn.query(
            ...
            (error, result, field) => {
              ...
              res.json({
                success_code: 200,
                data: { coin_balance: initialBalance },
              });
            }
          );
        }
    ...
    // 更新金币
    sqlStr =
      "UPDATE t_user_coins SET coin_balance = ?, total_earned = ? WHERE user_id = ?;";
    conn.query(
      sqlStr,
...
router.get("/api/admin/getUserCoinRecords", function (req, res) {
  let { userId, currentPage, pageSize } = req.query;
  ...
  let sqlStr =
    "SELECT * FROM t_coin_records WHERE user_id = ? ORDER BY created_at DESC;";
  let total;
  conn.query(sqlStr, [userId], (error, result, field) => {
    if (error) {
      console.log(error);
...
router.get("/api/admin/getCoinStatistics", function (req, res) {
  // 统计总金币发放量
  let sqlStr =
    "SELECT SUM(total_earned) as total_earned, COUNT(*) as user_count, SUM(coin_balance) as total_balance FROM t_user_coins;";
  conn.query(sqlStr, (error, result, field) => {
    if (error) {
      console.log(error);
      ...
      let today = new Date().toISOString().split("T")[0];
      sqlStr =
        "SELECT COUNT(*) as today_checkin FROM t_user_coins WHERE last_checkin_date = ?;";
 ...
 *   "userId": "123",               // 用户ID
 *   "taskId": "1",                 // 任务ID
 *   "orderId": "ORD1729757923456", // 订单ID（唯一标识）
 *   "coins": 100,                  // 奖励金币数
 *   "totalCount": 10,              // 任务总次数
 *   "completedCount": 3,           // 已完成次数
 *   "timestamp": 1729757923456,    // 时间戳（毫秒）
 *   "timezone": "Asia/Shanghai"    // 时区
 * }
 *
 * 返回格式（积分墙后端期望的格式）：
 * 成功: { success: true, message: "ok" | "奖励发放成功", data: {...} }
 * 失败: { success: false, message: "错误描述" }
 */
router.post("/api/task/callback", (req, res) => {
  ...
  // 1. 签名验证（必须先验证签名）
  const { verifyCallbackSignature } = require("../utils/signature");
  const verifyResult = verifyCallbackSignature(req.body);
  ...
  if (!userId || !orderId || coins === undefined || coins === null) {
    console.error("❌ 参数验证失败:", { userId, orderId, coins });
    return res.status(200).json({
      success: false,
      message: "参数缺失：userId、orderId、coins 为必填项",
    });
  }
  ...
  if (isNaN(coinsNum) || coinsNum < 0) {
    console.error("❌ 金币数量无效:", coins);
    return res.status(200).json({
      success: false,
      message: "金币数量无效，必须为非负数",
    });
  }
  ...
  // 3. 检查订单是否已处理（防重复）
  const checkOrderSql =
    "SELECT * FROM task_callback_records WHERE order_id = ?";
  conn.query(checkOrderSql, [orderId], (error, result) => {
    if (error) {
    ...
    if (result.length > 0) {
      ...
      console.log("⚠️ 订单已处理（幂等性），直接返回成功:", orderId);
    ...
    // 4. 更新用户金币
    console.log(
      `[回调] 开始更新用户 ${normalizedUserId} 的金币，增加 ${coinsNum}`
    );

    // 开启事务
    conn.beginTransaction((transErr) => {
      if (transErr) {
        console.error("❌ 开启事务失败:", transErr);
        return res.status(200).json({
          success: false,
          message: "事务开启失败",
        });
      ...
      // 4.1 更新用户金币
      // last_total_balance 记录金币变动前的余额
      const updateCoinsSql = `
        UPDATE t_user_coins 
        SET last_total_balance = coin_balance,
      ...
      conn.query(
        updateCoinsSql,
        [coinsNum, coinsNum, normalizedUserId],
        (updateErr, updateResult) => {
          if (updateErr) {
            console.error("❌ 更新金币失败:", updateErr);
            return conn.rollback(() => {
              res.status(200).json({
                success: false,
                message: "更新金币失败: " + updateErr.message,
              });
            });
          }

          console.log(
            `[回调] 数据库更新结果: affectedRows=${updateResult.affectedRows}`
          );

          // 如果没有更新任何行，说明用户不存在，需要先创建记录
          if (updateResult.affectedRows === 0) {
            console.log(
              `[回调] 用户 ${normalizedUserId} 金币记录不存在，创建新记录`
            );
          ...
          `;
            ...
            conn.query(
              insertCoinsSql,
              [normalizedUserId, coinsNum, coinsNum],
              (insertErr) => {
                if (insertErr) {
                  console.error("❌ 创建金币记录失败:", insertErr);
                  return conn.rollback(() => {
                    res.status(200).json({
                      success: false,
                      message: "创建金币记录失败: " + insertErr.message,
                    });
                  });
                }

                console.log(
                  `[回调] ✅ 用户金币记录创建成功，初始金币: ${coinsNum}`
                );
                // 继续记录回调日志
                recordCallback();
              }
            ...
            console.log(`[回调] ✅ 用户金币更新成功，增加 ${coinsNum} 金币`);
            // 更新成功，继续记录回调日志
            recordCallback();
          }
        }
      ...
      // 4.2 记录回调日志到 task_callback_records 表
      function recordCallback() {
        console.log(`[回调] 开始记录回调日志到 task_callback_records 表...`);

        const recordSql = `
          INSERT INTO task_callback_records 
          (order_id, user_id, task_id, reward_coins, total_count, completed_count, 
           callback_timestamp, timezone, status, created_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'success', NOW())
        `;
        ...
        conn.query(
          ...
          (recordErr) => {
            if (recordErr) {
              console.error("❌ 记录回调日志失败:", recordErr);
              return conn.rollback(() => {
                res.status(200).json({
                  success: false,
                  message: "记录回调日志失败: " + recordErr.message,
                });
              });
            }

            console.log(`[回调] ✅ 回调日志记录成功`);
            ...
            conn.commit((commitErr) => {
              if (commitErr) {
                ...
                return conn.rollback(() => {
                  res.status(200).json({
                    success: false,
                    message: "事务提交失败: " + commitErr.message,
                  });
                });
              }

              console.log(`[回调] ✅ 事务提交成功`);
              ...
              conn.query(
                ...
                (balanceErr, balanceResult) => {
                  if (balanceErr) {
                    console.error("❌ 查询金币余额失败:", balanceErr);
                    // 即使查询失败，回调也已成功，返回成功
                    const processTime = Date.now() - startTime;
                    return res.status(200).json({
                      success: true,
                  ...
                  balanceResult = JSON.parse(JSON.stringify(balanceResult));
                  const newBalance = balanceResult[0]?.coin_balance || 0;
                  const totalEarned = balanceResult[0]?.total_earned || 0;
                  const processTime = Date.now() - startTime;

                  console.log("✅ 回调处理成功!");
                  console.log(`   - 用户ID: ${normalizedUserId}`);
                  console.log(`   - 任务ID: ${taskId || "N/A"}`);
                  console.log(`   - 订单ID: ${orderId}`);
                  console.log(`   - 奖励金币: +${coinsNum}`);
                  console.log(`   - 当前余额: ${newBalance}`);
                  console.log(`   - 累计收益: ${totalEarned}`);
                  console.log(`   - 完成进度: ${completedCount}/${totalCount}`);
                  console.log(`   - 处理耗时: ${processTime}ms`);
                  console.log("========================================\n");
